...
# split qty across TP/SL to avoid locking 100% twice
half_qty = _round_step(free_btc * 0.48, step)  # ~48% each leg, leave ~4% dust

tp_price  = _round_tick(last * 1.01, tick)
sl_trig   = _round_tick(last * 0.99, tick)
sl_limit  = _round_tick(float(sl_trig) * 0.999, tick)

tp = c.new_order(symbol=symbol, side="SELL", type="LIMIT", timeInForce="GTC",
                 quantity=half_qty, price=tp_price)
sl = c.new_order(symbol=symbol, side="SELL", type="STOP_LOSS_LIMIT", timeInForce="GTC",
                 quantity=half_qty, price=sl_limit, stopPrice=sl_trig)

print({"tp_orderId": tp.get("orderId"), "sl_orderId": sl.get("orderId"),
       "tp_price": tp_price, "sl_trig": sl_trig, "sl_limit": sl_limit, "qty_each": half_qty})
