...
sell_frac=float(os.getenv("SELL_FRAC","1.0"))
base=sym.replace("USDT","")
acct=c.account()
free=next((float(b["free"]) for b in acct["balances"] if b["asset"]==base),0.0)
if free<=0:
    print(json.dumps({"status":"ERROR","msg":f"No {base} balance"})); sys.exit(1)
to_sell=free*sell_frac
...
print(json.dumps({"status":"OK","symbol":sym,"qty":qty,"orderId":o.get("orderId")}))
