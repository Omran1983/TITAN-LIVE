...
export class MauritianPayroll {
  ...
  generateDetailedPayslip(employee: Employee, payrollRecord: any): string {
    return `
...
OVERTIME:
Weekday Overtime (${payrollRecord.overtimeWeekdayHours}h @ 1.5x): Rs ${payrollRecord.overtimeWeekdayAmount.toFixed(2)}
Sunday Overtime (${payrollRecord.overtimeSundayHours}h @ 2.0x): Rs ${payrollRecord.overtimeSundayAmount.toFixed(2)}
Sunday Overtime (${payrollRecord.overtimeSundayTripleHours}h @ 3.0x): Rs ${payrollRecord.overtimeSundayTripleAmount.toFixed(2)}
Total Overtime: Rs ${payrollRecord.overtimeAmount.toFixed(2)}

ATTENDANCE DEDUCTIONS:
Absence Deduction: Rs ${payrollRecord.absenceDeduction?.toFixed(2) || '0.00'}
Lateness Deduction: Rs ${payrollRecord.latenessDeduction?.toFixed(2) || '0.00'}
Early Departure Deduction: Rs ${payrollRecord.earlyDepartureDeduction?.toFixed(2) || '0.00'}

GROSS SALARY: Rs ${payrollRecord.grossSalary.toFixed(2)}

EMPLOYEE DEDUCTIONS:
CSG (1.5%/3%): Rs ${payrollRecord.csgEmployee?.toFixed(2) || '0.00'}
...
Levy (1.5%): Rs ${payrollRecord.levy?.toFixed(2) || '0.00'}
PRGF (4.5%): Rs ${payrollRecord.prgfEmployee?.toFixed(2) || '0.00'}
...
CSG Employer (3%/6%): Rs ${payrollRecord.csgEmployer?.toFixed(2) || '0.00'}
NSF Employer (2.5%): Rs ${payrollRecord.nsfEmployer?.toFixed(2) || '0.00'}
NPF Employer (3%): Rs ${(payrollRecord.grossSalary * 0.03).toFixed(2)}
Total Employer Contributions: Rs ${(payrollRecord.totalEmployerContributions + payrollRecord.grossSalary * 0.03).toFixed(2)}
