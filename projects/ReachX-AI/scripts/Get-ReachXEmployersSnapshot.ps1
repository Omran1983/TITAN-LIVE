...
$endpoint = "$SupabaseUrl/rest/v1/employers?select=company_name,country,city,sector,headcount_estimate,contact_name,contact_phone,contact_email,created_at&order=created_at.desc&limit=50"
...
Write-Host "Fetching employers snapshot from: $endpoint" -ForegroundColor Cyan

try {
    $resp = Invoke-RestMethod -Uri $endpoint -Headers $headers -Method Get
}
catch {
    Write-Host "Error calling Supabase (employers): $($_.Exception.Message)" -ForegroundColor Red
    return
}

if (-not $resp) {
    Write-Host "No employers returned." -ForegroundColor Yellow
    return
}
